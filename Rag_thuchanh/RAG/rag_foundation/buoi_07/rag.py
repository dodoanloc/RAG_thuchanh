"""
Module RAG cho Buổi 07: Loader, Validator, Status, Indexing và Retrieval/Query.
Mọi đường dẫn sử dụng Path(__file__).resolve() để đảm bảo tính động.
"""
...
def load_config() -> dict:
    """
    Nạp và kiểm tra cấu hình từ file .env trong thư mục Buổi 07.
    """
...
def generate_embeddings(
    ...
    Tự động xử lý rate limit (HTTP 429) nếu gặp quota gián đoạn.
    """
    if client is None:
        cfg = load_config()
    ...
    # 9router accepts OpenAI-style batched embedding requests. Reduce requests
    # from one request/chunk to one request/64 chunks, avoiding its rate limit.
    if isinstance(client, RouterClient):
        for start in range(0, len(chunks), 8):
            batch = chunks[start:start + 8]
            texts = [f"title: {chunk['source']} | text: {chunk['text']}" for chunk in batch]
            response = None
            ...
            if response is None:
                raise RuntimeError("9router không trả embedding sau khi retry")
            vectors = [list(item.values) for item in response.embeddings]
            validate_embeddings(vectors, len(batch), dimension)
...
def main():
    parser = argparse.ArgumentParser(description="RAG Buổi 07 - Loader, Validator, Status, Indexing & Query")
    ...
    if args.command == "validate":
        try:
            ...
            print(f"=== METADATA MẪU (tối đa {num_samples} mẫu) ===")
            for i in range(num_samples):
                c = chunks[i]
                meta_sample = {k: v for k, v in c.items() if k != "text"}
                meta_sample["text_snippet"] = c["text"][:80] + "..." if len(c["text"]) > 80 else c["text"]
                print(f"Mẫu #{i+1}: {json.dumps(meta_sample, ensure_ascii=False)}")
        ...
        try:
            ...
            print(f"CÂU TRẢ LỜI:\n{res['answer']}")
            print("-" * 50)
            print(f"EVIDENCES RETRIEVED ({len(res['evidence'])} item):")
            for ev in res['evidence']:
                p_str = f"tr. {ev['page_start']}" if ev['page_start'] == ev['page_end'] else f"tr. {ev['page_start']}-{ev['page_end']}"
                acc_str = "ACCEPTED" if ev['accepted'] else "REJECTED (Distance > Threshold)"
                snip = ev['text'][:80].replace("\n", " ") + "..." if len(ev['text']) > 80 else ev['text'].replace("\n", " ")
                print(f" - [{ev['evidence_id']}] [{acc_str}] Dist: {ev['distance']:.4f} | Nguồn: {ev['source']} ({p_str}, chunk: {ev['chunk_id']})")
                print(f"   Preview: {snip}")

            if res["citations"]:
                print("-" * 50)
                print(f"CITATIONS MAPPED ({len(res['citations'])} item):")
                for cit in res["citations"]:
                    print(f" - {cit['evidence_id']}: {cit['display']}")

            if res["warnings"]:
                print("-" * 50)
                print(f"CẢNH BÁO ({len(res['warnings'])} item):")

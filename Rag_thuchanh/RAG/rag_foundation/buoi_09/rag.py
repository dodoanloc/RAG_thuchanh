"""
Module RAG cho Buổi 09: Semantic Baseline (sao chép độc lập từ Buổi 08).
Đóng vai trò Semantic Baseline để so sánh với Multi-query Retrieval và Parent-Child Retrieval Pipeline.
Mọi đường dẫn sử dụng Path(__file__).resolve() để tự quản lý cấu hình .env và storage riêng của Buổi 09.
"""
...
def load_config() -> dict:
    """
    Nạp và kiểm tra cấu hình từ file .env trong thư mục Buổi 09.
    """
...
def main():
    parser = argparse.ArgumentParser(description="RAG Buổi 09 - Baseline Loader, Validator, Status, Indexing & Query")
